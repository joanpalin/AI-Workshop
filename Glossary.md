...
**AnythingLLM** — A free, open-source desktop application that gives you a full AI assistant environment — chat interface, document upload, RAG, and agentic skills — all in one place, no coding required. We use it in Session 3 to build our agentic AI workflow. Download it once, and it runs entirely on your laptop.
...
**Groq** — A cloud-based AI service that provides access to large, powerful language models at no cost on its free tier. In Session 3, we use Groq to run a model called Llama 3.3 70B — a much larger model than the one on your laptop — because reliable multi-step agentic tasks benefit from greater reasoning capability. No credit card is required to sign up.
...
**LM Studio** — A free desktop application that makes it easy to download and run AI models locally on your own laptop — no cloud, no API key, no data leaving the building. It also has a built-in server mode that allows other tools (like MCP servers) to connect to the local model. We use it in Session 2 to power our AI assistant.
...
**Node.js** — A lightweight, free software runtime that allows certain tools to run on your laptop. You won't write a single line of Node.js code today — we just need it installed so that LM Studio can load MCP servers in Session 2. Think of it as a quiet engine running in the background.

**Qwen 2.5 3B Instruct** — The specific AI model we download into LM Studio for Session 2. It was built by Alibaba's Qwen research team, is fully open-source, weighs roughly 1.9 GB, and runs smoothly on a standard corporate laptop without a dedicated graphics card. "3B" refers to its three billion parameters — small enough to be portable, capable enough for our assistant tasks.

**Tavily** — A web-search service designed specifically for AI assistants. Where a standard search engine gives you ten blue links, Tavily returns clean, summarised information the AI can read directly. It has a generous free tier (1,000 searches per month, email sign-up only, no credit card). We connect it to the AI in Session 2 via MCP, and re-use it in Session 3 through AnythingLLM's built-in web-search skill.
