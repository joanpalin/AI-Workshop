...
> **How to use this page:** These prompts are designed for **Phase 7** (iteration) and **Phase 8** (table discussion and Agent Design Canvas) of the Session 3 lab. They range from Starter level — simple, single-step retrieval — to Advanced level, which asks the agent to plan and execute a multi-step research, comparison, and drafting workflow. Copy any prompt directly into your **ARK Industries Agent** workspace chat window and press Enter. You do not need to type anything from scratch.
...
**Difficulty:** Starter
**What the agent will do:** Retrieve a specific policy rule from the uploaded `it-policy-summary.txt` document and summarise it in plain language.
...
**What the agent will do:** Search the web for current UK government data classification standards (such as the Government Security Classifications or ICO guidance), read the ARK Industries policy, and identify whether the internal classification scheme aligns with current guidance.
...
**Difficulty:** Intermediate
**What the agent will do:** Search for current best-practice guidance on remote access security (e.g., zero trust network access, MFA requirements), compare against the ARK Industries remote access policy in Section 4, and identify where the policy may be lagging behind current practice.
...
**Difficulty:** Intermediate
**What the agent will do:** Identify whether the ARK Industries Acceptable Use policy (Section 1) addresses the use of AI tools, search for UK employer guidance on AI acceptable use policies, and flag the gap — this prompt is designed to surface the fact that the current policy predates widespread enterprise AI adoption.
...
**Difficulty:** Advanced
**What the agent will do:** Search for current incident response maturity frameworks (NIST, NCSC), compare the ARK Industries incident reporting policy (Section 6) and its reference to the Incident Response Plan against current framework requirements, identify specific maturity gaps, draft a structured assessment with recommended improvements, and pause for approval.
...
In every case, your job is the same: set the goal, watch the thought chain, review the output, and decide. The agent does the legwork. You make the call.
